...
impl<'a> Surt<'a> {
    ...
    /// canonical URL rather than the original one. The returned value implements [`fmt::Display`]
    /// and renders on demand; call `to_string` or pass it to a formatting macro such as `write!`.
    #[must_use]
    ...
    ///
    /// [`url`](Self::url) renders the same text; this method parses it for APIs that require a
    /// [`fluent_uri::Uri`], including builders for URI-valued WARC headers in `archivindex-warc`.
    ///
    /// # Errors
    ///
...
/// The URL a key stands for, rendered on demand by [`Surt::url`].
struct Url<'s, 'a> {
    key: &'s Surt<'a>,
    scheme: &'s str,
}
