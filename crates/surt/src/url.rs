...
//!
//! [`Url`] is a lightweight structural parser for
//! `scheme://[userinfo@]host[:port][/path][?query][#fragment]`. It checks the scheme, authority,
//! host, and port and rejects whitespace and control characters, but does not otherwise validate
//! each component against a URL standard. It can render the host-reordering transforms
//! [`Url::surt`], [`Url::heritrix`], and [`Url::ssurt`]. A [`Canonicalizer`] first applies a
//! convention's normalization rules so that equivalent URLs render the same.
...
///
/// Regardless of its flags, every canonicalizer:
///
/// - repairs input as the Python library does by trimming whitespace, removing tabs and newlines,
///   defaulting to `http://`, and unwrapping `http://https://...`;
/// - lowercases the scheme, host, path, and query;
/// - drops user information, fragments, default ports, and empty queries;
/// - resolves path dot segments and strips trailing dots from hosts;
/// - renders numeric IPv4 hosts as dotted quads and non-ASCII hosts with IDNA.
#[expect(
    clippy::struct_excessive_bools,
    reason = "each boolean independently enables one optional normalization rule"
)]
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
...
/// Borrow the URI's text and record its component offsets without canonicalizing it.
...
/// Fails when the stored text is not a URI. [`Url::parse`] performs structural checks but permits
/// characters such as `{` or `|` in paths, which RFC 3986 does not allow.
#[cfg(feature = "fluent-uri")]
#[cfg_attr(docsrs, doc(cfg(feature = "fluent-uri")))]
...
/// Parse the stored text with the WHATWG URL parser.
///
/// The WHATWG parser accepts more input than [`Url::parse`] but may re-encode it, so the resulting
/// text can differ.
#[cfg(feature = "url")]
#[cfg_attr(docsrs, doc(cfg(feature = "url")))]
