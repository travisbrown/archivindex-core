//! CDXJ records and their JSON field objects.
...
pub struct Fields<'a> {
    ...
    #[serde(
        default,
        deserialize_with = "crate::format::optional_integer",
        serialize_with = "crate::format::optional_integer_str",
        skip_serializing_if = "Option::is_none"
    )]
...
pub struct ConformingFields<'a> {
    ...
    mime: Cow<'a, str>,
    /// The HTTP response status.
    #[serde(serialize_with = "crate::format::integer_str")]
    status: u16,
    /// The WARC record offset.
    #[serde(serialize_with = "crate::format::integer_str")]
    offset: u64,
...
mod tests {
    ...
    fn complete_fields() -> ConformingFields<'static> {
        ConformingFields::new(
            "https://example.com/",
            "sha256:payload",
            "text/html",
            200,
    ...
    #[test]
    fn rejects_truncated_and_invalid_lines() {
        for line in [
            "",
            "key",
            "key timestamp",
            ...
            assert!(matches!(Item::parse(line), Err(Error::Truncated(value)) if value == line));
        }

        assert!(matches!(
            Item::parse("key 20201007212236 {"),
            Err(Error::InvalidFields(_))
        ));
    }

    #[test]
    fn from_str_owns_the_item() -> Result<(), Error> {
        let item = EXAMPLE.parse::<Item<'static>>()?;

        assert!(matches!(item.key, Cow::Owned(_)));
        assert!(matches!(item.fields.url, Cow::Owned(_)));
        Ok(())
    }

    #[test]
    fn conforming_accessors_and_builders_expose_every_value() {
        let mut extra = ExtraProperties::default();
        extra.insert("custom".to_owned(), true.into());
        let fields = complete_fields()
            .with_record_digest("sha256:record")
            .with_extra(extra)
            .unwrap();

        assert_eq!(fields.url(), "https://example.com/");
        assert_eq!(fields.digest(), "sha256:payload");
        assert_eq!(fields.mime(), "text/html");
        assert_eq!(fields.status(), 200);
        assert_eq!(fields.offset(), 784);
        assert_eq!(fields.length(), 1300);
        assert_eq!(fields.filename(), "data.warc.gz");
        assert_eq!(fields.record_digest(), Some("sha256:record"));
        assert_eq!(fields.extra()["custom"], true);

        let owned = fields.into_owned();
        assert!(matches!(owned.url, Cow::Owned(_)));
        assert!(matches!(owned.digest, Cow::Owned(_)));
        assert!(matches!(owned.mime, Cow::Owned(_)));
        assert!(matches!(owned.filename, Cow::Owned(_)));
        assert!(matches!(owned.record_digest, Some(Cow::Owned(_))));
    }

    #[test]
    fn items_convert_to_conforming_items_and_captures() -> Result<(), Box<dyn std::error::Error>> {
        let item = Item::parse(EXAMPLE)?;
        let conforming = ConformingItem::try_from(&item)?;

        assert_eq!(conforming.key, item.key);
        assert_eq!(conforming.fields.status(), 200);

        let capture = Capture::from(item);
        ...
        assert_eq!(capture.robot_flags, None);
    ...
    #[cfg(feature = "bounded-static")]
    #[test]
    fn bounded_static_conversions_detach_every_model() -> Result<(), Box<dyn std::error::Error>> {
        use bounded_static::{IntoBoundedStatic as _, ToBoundedStatic as _};

        let item = Item::parse(EXAMPLE)?;
        let fields = item.fields.clone();
        assert_eq!(fields.to_static(), fields.clone().into_owned());
        assert_eq!(fields.clone().into_static(), fields.into_owned());

        let conforming_fields = ConformingFields::try_from(&item.fields)?;
        assert_eq!(
            conforming_fields.to_static(),
            conforming_fields.clone().into_owned()
        );
        assert_eq!(
            conforming_fields.clone().into_static(),
            conforming_fields.into_owned()
        );

        assert_eq!(item.to_static(), item.clone().into_owned());
        assert_eq!(item.clone().into_static(), item.clone().into_owned());

        let conforming_item = ConformingItem::try_from(&item)?;
        assert_eq!(
            conforming_item.to_static(),
            conforming_item.clone().into_static()
        );
        Ok(())
    }
    ...
    fn conforming_fields_survive_the_lenient_form(
        ...
        prop_assert_eq!(
            ConformingFields::try_from(&lenient).ok(),
            Some(fields.clone())
        );
