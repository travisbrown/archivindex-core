//! JSON output from the Internet Archive CDX Server API.
//!
//! This representation is a JSON array whose first row names the fields and whose remaining rows
//! contain string values. A resumable response may end with an empty row followed by a
//! single-element row containing the resume key.
...
struct Row<'a>(#[serde(borrow, with = "archivindex_serde::borrowable_str::seq")] Vec<Cow<'a, str>>);
...
mod tests {
    ...
    use super::*;
    use crate::format::classic::Header;
